...
GMOD_MODULE_OPEN()
{
    ...
    #ifdef WIN32
    hostfxr_pointer = LoadLibraryA("garrysmod/lua/bin/dotnet/hostfxr.dll");
    #elif __APPLE__
    hostfxr_pointer = dlopen("garrysmod/lua/bin/dotnet/libhostfxr.dylib", RTLD_LAZY);
    #else
    hostfxr_pointer = dlopen("garrysmod/lua/bin/dotnet/libhostfxr.so", RTLD_LAZY);
    #endif
...
GMOD_MODULE_CLOSE()
{
    ...
    #ifdef WIN32
    HMODULE hostfxr_lib = LoadLibraryA("garrysmod/lua/bin/dotnet/hostfxr.dll");
    hostfxr_close = (hostfxr_close_fn)GetProcAddress(hostfxr_lib, "hostfxr_close");
    #elif __APPLE__
    void * hostfxr_lib = dlopen("garrysmod/lua/bin/dotnet/libhostfxr.dylib", RTLD_LAZY);
    hostfxr_close = (hostfxr_close_fn)dlsym(hostfxr_lib, "hostfxr_close");
    #else
    void * hostfxr_lib = dlopen("garrysmod/lua/bin/dotnet/libhostfxr.so", RTLD_LAZY);
    hostfxr_close = (hostfxr_close_fn)dlsym(hostfxr_lib, "hostfxr_close");
    #endif
